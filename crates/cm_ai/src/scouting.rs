...
fn calculate_true_player_score(player: &Player) -> u8 {
    ...
    let score =
        (ability as u16 * 50 + potential as u16 * 30 + form as u16 * 10 + fitness as u16 * 10)
            / 100;
...
pub fn generate_scout_report(
    ...
    let recommendation =
        generate_recommendation(current_ability, potential_ability, value_assessment, player);
...
pub fn compare_players(
    ...
    Some(PlayerComparison {
        ...
        value_winner: if a.value <= b.value {
            player_a.clone()
        } else {
            player_b.clone()
        },
...
mod tests {
    ...
    fn test_scout_score_better_with_better_scout() {
        ...
        assert!(
            score_high >= 70,
            "Elite player should score high: {}",
            score_high
        );
        assert!(
            score_low >= 50,
            "Even low scout should see quality: {}",
            score_low
        );
    }
    ...
    fn test_generate_scout_report() {
        ...
        assert!(
            report.potential_ability > report.current_ability,
            "Young prospect should have high potential"
        );
    }
    ...
    fn test_value_assessment_bargain() {
        ...
        assert!(
            matches!(
                ...
                ValueAssessment::Bargain | ValueAssessment::Undervalued
            ),
    ...
    fn test_recommendation_must_sign() {
        ...
        assert!(
            matches!(
                ...
                ScoutRecommendation::MustSign | ScoutRecommendation::Recommended
            ),
    ...
    fn test_recommendation_young_prospect() {
        ...
        assert!(
            matches!(
                report.recommendation,
                ScoutRecommendation::MustSign
                    | ScoutRecommendation::Recommended
                    | ScoutRecommendation::Watchlist
            ),
        ...
        for id in &results {
            ...
            assert!(
                player.position == Position::ForwardCenter
                    || player
                        .secondary_positions
                        .contains(&Position::ForwardCenter)
            );
        }
    }
