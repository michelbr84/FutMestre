...
fn average_age_for_position(
    players: &[&Player],
    position: Position,
    current_year: i32,
) -> Option<u8> {
    let position_players: Vec<_> = players.iter().filter(|p| p.position == position).collect();
...
pub fn analyze_squad_strength(world: &World, club_id: &ClubId) -> SquadStrength {
    ...
    let midfielders: Vec<_> = players
        .iter()
        .filter(|p| p.position.is_midfielder())
        .collect();

    let forwards: Vec<_> = players.iter().filter(|p| p.position.is_forward()).collect();
    ...
    let overall = (goalkeeper_strength as u16 * 1
...
mod tests {
    ...
    fn create_test_player(id: &str, position: Position, quality: u8, age_years: i32) -> Player {
    ...
    fn test_analyze_weak_squad() {
        ...
        let critical_needs: Vec<_> = needs
            .iter()
        ...
        let fullback_needs: Vec<_> = needs
            .iter()
    ...
    fn test_analyze_balanced_squad() {
        ...
        assert!(
            critical_needs.is_empty(),
            "Balanced squad should have no critical needs"
        );
    }
