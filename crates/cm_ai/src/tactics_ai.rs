...
use cm_core::world::{Formation, Mentality, Position, Tactics, Tempo, TrainingFocus};
...
pub fn adjust_tempo(score_diff: i8, minutes_remaining: u8, average_fitness: u8) -> Tempo {
...
pub fn recommend_pressing(score_diff: i8, minutes_remaining: u8, average_fitness: u8) -> u8 {
...
pub fn recommend_defensive_line(formation: Formation, opponent_pace: u8, own_pace: u8) -> u8 {
...
/// Recomenda foco de treino baseado na posicao do jogador.
///
/// Goleiros focam em tecnica (reflexos), defensores e volantes em defesa,
/// meias em tecnica, meias ofensivos e atacantes em ataque.
pub fn recommend_training_focus(position: Position) -> TrainingFocus {
    match position {
        Position::Goalkeeper => TrainingFocus::Technical,
        Position::DefenderCenter | Position::DefenderLeft | Position::DefenderRight => {
            TrainingFocus::Defending
        }
        Position::MidfielderDefensive => TrainingFocus::Defending,
        Position::MidfielderCenter | Position::MidfielderLeft | Position::MidfielderRight => {
            TrainingFocus::Technical
        }
        Position::MidfielderAttacking => TrainingFocus::Attacking,
...
/// IA tatica adaptativa durante a partida.
/// Analisa o placar, tempo restante e fitness para sugerir mudancas taticas.
///
/// Retorna a mentalidade e o ritmo recomendados com base no estado atual do jogo.
pub fn adaptive_match_tactics(
    score_diff: i8,
    minutes_played: u8,
    minutes_total: u8,
    average_fitness: u8,
    current_mentality: Mentality,
    current_tempo: Tempo,
) -> (Mentality, Tempo) {
    let minutes_remaining = minutes_total.saturating_sub(minutes_played);

    // Low fitness override: slow down tempo regardless of situation
    if average_fitness < 40 {
        let mentality = adjust_mentality(score_diff, minutes_remaining);
        return (mentality, Tempo::Slow);
    }

    // Winning by 2+ with < 20 min left: go Defensive + Slow
    if score_diff >= 2 && minutes_remaining < 20 {
        return (Mentality::Defensive, Tempo::Slow);
    }

    // Losing with < 15 min left: AllOutAttack + Fast
    if score_diff < 0 && minutes_remaining < 15 {
        return (Mentality::AllOutAttack, Tempo::Fast);
    }

    // Losing with < 30 min left: Attacking + Fast
    if score_diff < 0 && minutes_remaining < 30 {
        return (Mentality::Attacking, Tempo::Fast);
    }

    // Drawing in final 10 min: Attacking + Normal
    if score_diff == 0 && minutes_remaining <= 10 {
        return (Mentality::Attacking, Tempo::Normal);
    }

    // Otherwise keep current
    (current_mentality, current_tempo)
}
...
mod tests {
    ...
    fn test_recommend_tactics() {
        ...
        assert!(matches!(
            tactics.formation,
            Formation::F433 | Formation::F4231
        ));
    ...
    #[test]
    fn test_adaptive_winning_comfortably_late() {
        // Winning by 2 with 10 min left -> Defensive + Slow
        let (m, t) = adaptive_match_tactics(2, 80, 90, 70, Mentality::Balanced, Tempo::Normal);
        assert_eq!(m, Mentality::Defensive);
        assert_eq!(t, Tempo::Slow);
    }

    #[test]
    fn test_adaptive_losing_late() {
        // Losing with < 15 min left -> AllOutAttack + Fast
        let (m, t) = adaptive_match_tactics(-1, 78, 90, 70, Mentality::Balanced, Tempo::Normal);
        assert_eq!(m, Mentality::AllOutAttack);
        assert_eq!(t, Tempo::Fast);
    }

    #[test]
    fn test_adaptive_losing_mid_late() {
        // Losing with < 30 min left (but >= 15) -> Attacking + Fast
        let (m, t) = adaptive_match_tactics(-1, 65, 90, 70, Mentality::Balanced, Tempo::Normal);
        assert_eq!(m, Mentality::Attacking);
        assert_eq!(t, Tempo::Fast);
    }

    #[test]
    fn test_adaptive_drawing_final_minutes() {
        // Drawing in final 10 min -> Attacking + Normal
        let (m, t) = adaptive_match_tactics(0, 82, 90, 70, Mentality::Balanced, Tempo::Normal);
        assert_eq!(m, Mentality::Attacking);
        assert_eq!(t, Tempo::Normal);
    }

    #[test]
    fn test_adaptive_low_fitness_slows_tempo() {
        // Low fitness should force Slow tempo regardless
        let (_, t) = adaptive_match_tactics(-1, 70, 90, 35, Mentality::Attacking, Tempo::Fast);
        assert_eq!(t, Tempo::Slow);
    }

    #[test]
    fn test_adaptive_keeps_current_when_no_trigger() {
        // Early in the match, no special conditions -> keep current
        let (m, t) = adaptive_match_tactics(0, 20, 90, 75, Mentality::Balanced, Tempo::Normal);
        assert_eq!(m, Mentality::Balanced);
        assert_eq!(t, Tempo::Normal);
    }

    #[test]
    fn test_recommend_training_focus_goalkeeper() {
        assert_eq!(
            recommend_training_focus(Position::Goalkeeper),
            TrainingFocus::Technical
        );
    }

    #[test]
    fn test_recommend_training_focus_defenders() {
        assert_eq!(
            recommend_training_focus(Position::DefenderCenter),
            TrainingFocus::Defending
        );
        assert_eq!(
            recommend_training_focus(Position::DefenderLeft),
            TrainingFocus::Defending
        );
        assert_eq!(
            recommend_training_focus(Position::DefenderRight),
            TrainingFocus::Defending
        );
        assert_eq!(
            recommend_training_focus(Position::MidfielderDefensive),
            TrainingFocus::Defending
        );
    }

    #[test]
    fn test_recommend_training_focus_midfielders() {
        assert_eq!(
            recommend_training_focus(Position::MidfielderCenter),
            TrainingFocus::Technical
        );
        assert_eq!(
            recommend_training_focus(Position::MidfielderLeft),
            TrainingFocus::Technical
        );
        assert_eq!(
            recommend_training_focus(Position::MidfielderRight),
            TrainingFocus::Technical
        );
    }

    #[test]
    fn test_recommend_training_focus_attackers() {
        assert_eq!(
            recommend_training_focus(Position::MidfielderAttacking),
            TrainingFocus::Attacking
        );
        assert_eq!(
            recommend_training_focus(Position::ForwardCenter),
            TrainingFocus::Attacking
        );
        assert_eq!(
            recommend_training_focus(Position::ForwardLeft),
            TrainingFocus::Attacking
        );
        assert_eq!(
            recommend_training_focus(Position::ForwardRight),
            TrainingFocus::Attacking
        );
    }
}
