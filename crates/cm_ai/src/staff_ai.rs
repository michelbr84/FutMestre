...
pub fn should_hire_staff(world: &World, club_id: &ClubId) -> bool {
    ...
    needs
        .iter()
        .any(|n| matches!(n.priority, StaffPriority::Critical | StaffPriority::High))
}
...
pub fn analyze_staff_needs(world: &World, club_id: &ClubId) -> Vec<StaffNeed> {
    ...
    if coach_count < MIN_COACHES {
        needs.push(StaffNeed {
            ...
            reason: format!(
                "Only {} coaches, need at least {}",
                coach_count, MIN_COACHES
            ),
        });
        ...
        needs.push(StaffNeed {
            ...
            reason: format!(
                "Only {} coaches, ideally need {}",
                coach_count, IDEAL_COACHES
            ),
        });
    }
    ...
    if physio_count < MIN_PHYSIOS {
        ...
        needs.push(StaffNeed {
            ...
            priority: StaffPriority::Low,
            reason: format!(
                "Only {} physio(s), could use {}",
                physio_count, IDEAL_PHYSIOS
            ),
        });
    }
...
fn get_staff_ability(staff: &Staff) -> u8 {
    match staff.role {
        ...
        StaffRole::Scout => staff.scouting,
        StaffRole::Physio => staff.physiotherapy,
        StaffRole::FitnessCoach => staff.fitness,
        StaffRole::Manager => {
            ((staff.coaching as u16 + staff.man_management as u16 + staff.tactics as u16) / 3) as u8
        }
        StaffRole::DataAnalyst => staff.tactics,
    }
}
...
pub fn evaluate_staff_hire(
    ...
    let needs = analyze_staff_needs(world, club_id);
    let role_needed = needs.iter().find(|n| matches_role(&n.role, &staff.role));

    let ability = get_staff_ability(staff);

    match role_needed {
        Some(need) if need.priority >= StaffPriority::High => HireDecision::Hire {
...
pub fn identify_releasable_staff(world: &World, club_id: &ClubId) -> Vec<(StaffId, ReleaseReason)> {
    ...
    let mut role_counts: std::collections::HashMap<StaffRole, usize> =
...
mod tests {
    ...
    fn create_test_staff(id: &str, role: StaffRole, quality: u8) -> Staff {
        let mut staff = Staff::new(id, "Test", id, NationId::new("test"), role);
    ...
    fn test_evaluate_staff_hire_not_needed() {
        ...
        assert!(matches!(
            decision,
            HireDecision::Consider { .. } | HireDecision::Reject { .. }
        ));
    }
    ...
    fn test_identify_releasable_staff() {
        ...
        world
            ...
            .unwrap()
            .staff_ids
            .push(bad_coach.id.clone());
        world.staff.insert(bad_coach.id.clone(), bad_coach);

        let releasable = identify_releasable_staff(&world, &club_id);

        let has_bad_coach = releasable.iter().any(|(id, reason)| {
            *id == StaffId::new("bad_coach") && *reason == ReleaseReason::LowQuality
        });
    ...
    fn test_calculate_coaching_quality() {
        ...
        assert!(
            quality >= 60,
            "Full coaching staff should have good quality: {}",
            quality
        );
    }
    ...
    fn test_calculate_scouting_quality() {
        ...
        assert!(
            quality >= 60,
            "Full scouting network should have good quality: {}",
            quality
        );
    }
    ...
    fn test_calculate_medical_quality() {
        ...
        assert!(
            quality >= 70,
            "Full medical staff should have good quality: {}",
            quality
        );
    }
    ...
    fn test_needs_sorted_by_priority() {
        ...
        for i in 1..needs.len() {
            assert!(
                needs[i - 1].priority >= needs[i].priority,
